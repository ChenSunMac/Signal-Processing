function [tm, numberOfRec, header] = importDataVersion0(obj, h, fileId)
...
%             header.transducerType;  uint16
    ...
    header.pulsePattern = 1;   % Default for version 0   
    header.pulseLength = 400;  % Default for version 0
    ...
    for n = 1:h.numRec;                                
        ...
        %
        [tm(n).xPos, count] = fread(fileId, 1,'int32');
        if count < 1
            error(errorMsg);
        end

        [tm(n).yPos, count] = fread(fileId, 1,'int32');
        if count < 1
            error(errorMsg);
        end

        [tm(n).zPos, count] = fread(fileId, 1,'int32');
        if count < 1
            error(errorMsg);
        end
